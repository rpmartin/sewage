...
If you had reason to believe that every combination of discount rate and time horizon were equally likely occur you could easily calculate the average net present value associated with each of the plant types:

```{r}
ave_results <- results%>%
  group_by(plant)%>%
  summarize(average_net_present_value=mean(net_present_value))%>%
  arrange(desc(average_net_present_value))

kbl(ave_results,col.names=formatter(colnames(ave_results)),digits=1)%>% 
